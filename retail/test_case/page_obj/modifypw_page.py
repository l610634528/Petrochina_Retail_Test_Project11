...
class PrimaryMenu(BasePage):
    ...
    pwdList = \
        [[modifyPwData.readExcel(1, 0), modifyPwData.readExcel(1, 1), modifyPwData.readExcel(1, 2)],
         ...
         [modifyPwData.readExcel(5, 0), modifyPwData.readExcel(5, 2), modifyPwData.readExcel(5, 2)]]
    ...
    def modifyPwMenu(self):
        ...
        try:
            ...
            log.logger.exception(
                'not found menu [%s]-[%s]-[%s]' % (self.menuList[0], self.menuPersonal, self.menuModifyPwd))
            raise
        else:
    ...
    def errorDialog(self, commit_btn=(By.ID, 'unieap_form_Button__unieap_input')):
        """
        :type commit_btn: 元祖
        """
